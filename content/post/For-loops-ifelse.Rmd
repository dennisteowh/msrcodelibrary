...
The logic is a simple extension of a single **for loop**. The first loop (or outer loop) iterates on i from 1 to 3. On each iteration, the second loop (or inner loop) runs and iterates on j from "a" to "c". Hence, the first 3 outputs are "1a", "1b", and "1c" which takes place within the first iteration of the outer loop.  
...
Even though nested for loops are a mere extension of for loops, it significantly increases your capabilities to solve complex problems. However, it can also be confusing to conceptualize the nesting so it is important you try to practice this on your own. In the next section, I share a challenge where you can test out your new skills in R programming.  
